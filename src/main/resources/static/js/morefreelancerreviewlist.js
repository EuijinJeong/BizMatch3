$().ready(function () {
  ...
  var email = window.location.pathname.split("/").pop();
  let orderBy = "late-date";
  $("#orderBy").val(orderBy);

  // 초기 AJAX 호출로 리뷰 목록 로드
  callList(
    `/member/mypage/company/reviews/${email}`,
    { orderBy },
    0,
    10,
    function (response) {
      console.log("초기 로드 Response:", response);
      displayReviews(response.reviewList);
    }
  );
  ...
  $(".sort-option").on("click", function () {
    ...
    callList(
      `/member/mypage/company/${email}/reviews`,
      { orderBy },
      0,
      10,
      function (response) {
